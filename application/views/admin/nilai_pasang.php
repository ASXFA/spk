<div class="container-fluid">
    <div class="card shadow mb-4">
        <div class="card-header">
            <h5 class="d-inline">Konversi Nilai Kategori</h5>
            <span class="float-right">
                ...
                <script>
                    $('#btnReset').on('click', function (e) {
                        e.preventDefault();
                        const url = '<?= base_url('dashboard-admin/manage-nilai/hasil-nilai') ?>';
                        swal.fire({
        ...
        <div class="card-body">
            <table class="table table-borderless">
                <thead>
                    ...
                    <th width="15%">Nilai Subprioritas</th>
                </thead>
                <tbody>
                    <tr>
                        <td></td>
                    ...
                    ?>
                                ...
                                <td>
                                    <input type="text" class="form-control form-control-sm" id="hasilsub1" readonly>
                                </td>    
                            </tr>
                    <?php
                            }else if($no == 2){
                    ...
                    ?>
                                    ...
                                    <td>
                                        <input type="text" class="form-control form-control-sm" id="hasilsub2" readonly>
                                    </td>    
                                </tr>
                    <?php
                            }else if($no==3){
                    ...
                    ?>
                                    ...
                                    <td>
                                    <input type="text" class="form-control form-control-sm" id="hasilsub3" readonly>
                                </td>    
                                </tr>
                    <?php
                            }else if($no == 4){
                    ...
                    ?>
                                ...
                                <td>
                                    <input type="text" class="form-control form-control-sm" id="hasilsub4" readonly>
                                </td>    
                    <?php                  
                            }
                            $no++;
...
<script>
    $(function(){
        ...
        $.ajax({
            ...
            async:true,
            dataType:'JSON',
            url:'<?= base_url('dashboard-admin/manage-nilai/hasil-nilai/ambil') ?>',
            success:function(datak){
                if(datak.length === 0){
                    ...
                    for(i; i<datak.length; i++){
                        if(i===0){
                            $('#1-1').val(datak[i].nilai_pasangan);
                        }else if(i===1){
                            $('#2-1').val(datak[i].nilai_pasangan);
                        }else if(i===2){
                            $('#3-1').val(datak[i].nilai_pasangan);
                        }else if(i===3){
                            $('#4-1').val(datak[i].nilai_pasangan);
                            $('#hasil1').val(datak[i].prioritas);
                            $('#hasilsub1').val(datak[i].subprioritas);
                        }else if(i===4){
                            $('#1-2').val(datak[i].nilai_pasangan);
                            $('#1-2').attr('disabled','disabled');
                        }else if(i===5){
                            $('#2-2').val(datak[i].nilai_pasangan);
                        }else if(i===6){
                            $('#3-2').val(datak[i].nilai_pasangan);
                        }else if(i===7){
                            $('#4-2').val(datak[i].nilai_pasangan);
                            $('#hasil2').val(datak[i].prioritas);
                            $('#hasilsub2').val(datak[i].subprioritas);
                        }else if(i===8){
                            $('#1-3').val(datak[i].nilai_pasangan);
                            $('#1-3').attr('disabled','disabled');
                        }else if(i===9){
                            $('#2-3').val(datak[i].nilai_pasangan);
                            $('#2-3').attr('disabled','disabled');
                        }else if(i===10){
                            $('#3-3').val(datak[i].nilai_pasangan);
                        }else if(i===11){
                            $('#4-3').val(datak[i].nilai_pasangan);
                            $('#hasil3').val(datak[i].prioritas);
                            $('#hasilsub3').val(datak[i].subprioritas);
                        }else if(i===12){
                            $('#1-4').val(datak[i].nilai_pasangan);
                            $('#1-4').attr('disabled','disabled');
                        }else if(i===13){
                            $('#2-4').val(datak[i].nilai_pasangan);
                            $('#2-4').attr('disabled','disabled');
                        }else if(i===14){
                            $('#3-4').val(datak[i].nilai_pasangan);
                            $('#3-4').attr('disabled','disabled');
                        }else if(i===15){
                            $('#4-4').val(datak[i].nilai_pasangan);
                            $('#hasil4').val(datak[i].prioritas);
                            $('#hasilsub4').val(datak[i].subprioritas);
    ...
    function hapus(){
        $.ajax({
            method:'post',
            url:'<?= base_url('dashboard-admin/manage-nilai/hasil-nilai/hapus') ?>',
            data:{table1:'nilai_pasang',table2:'nilai_prioritas'},
            success:function(){
            }
        })
    }
    ...
    function hitung(){
        ...
        var nilaiSubprioritas = hitungSub(nilaiPrioritas);
        Swal.fire({
            position: 'top-end',
            icon: 'info',
            title: 'Loading !',
        ...
        for(k; k<nilaiPrioritas.length; k++){
            $.ajax({
                type:'POST',
                dataType:'json',
                url:'<?= base_url('dashboard-admin/manage-nilai/hasil-nilai/nilai-prioritas/tambah') ?>',
                data:{nilai_id:dataID[k],prioritas:nilaiPrioritas[k],subprioritas:nilaiSubprioritas[k]},
                success:function(){
                }
            });
        }
        ...
        if (CRFix < 0.1) {
                ...
                if(o==0){
                    ...
                    for(p; p<data2.length; p++){
                        ...
                        $.ajax({
                            type:'post',
                            dataType:'JSON',
                            url:'<?= base_url('dashboard-admin/manage-nilai/hasil-nilai/tambah') ?>',
            ...
            Swal.fire({
                ...
                window.location.href='<?= base_url('dashboard-admin/manage-nilai/hasil-nilai') ?>';
            })
    ...
    function hitungSub(arrs){
        var h=0;
        var besar=0;
        for(h; h<arrs.length; h++){
            if(arrs[h] > besar ){
                besar = arrs[h];
            }
        }
        ...
        var arrHasil = [];
        var g=0;
        for(g; g<arrs.length; g++){
            var hitung = arrs[g] / besar;
            var hitungFix = hitung.toFixed(2);
            arrHasil.push(parseFloat(hitungFix));
        }
        return arrHasil;
    }
